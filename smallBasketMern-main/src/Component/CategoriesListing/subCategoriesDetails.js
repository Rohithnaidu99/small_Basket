...
const SubCategoriesDetails = () => {
    ...
    useEffect(() => {
        const fetchSubCategories = async () => {
            try {
                let sub_category_name = params.sub_category_name;
                const response = await fetch(`${api_url}/subcategories/${sub_category_name}`, { method: "GET" });
                const data = await response.json();
                setSubCategories(data);
                setLoading(false);
                setSubCategoryName(data.length > 0 ? data[0].sub_category_name : "")
            } catch (error) {
                // Handle fetch error
                console.error(error);
            }
        ...
        <>
            {loading ? (
                ...
                <div className="container-fluid shadow-sm mt-3">
                    <h3 className="text-center">{subCategoryName}</h3>
                    <hr className="hr" />
                    <div className="row d-flex justify-content-center">
                        {renderData(subCategories)}
                    </div>
                </div>
            )}
        </>
